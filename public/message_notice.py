...
SLACK_WEBHOOK = MSG.get('SLACK').get('slack_webhook')
...
class SlackNotice:
    ...
    def send_text(content):
        ...
        try:
            rep = requests.post(url=send_url, headers=headers, data=send_data)
            logger.info('slack消息推送成功！', rep.text)
        except Exception as e:
            logger.error('slack消息推送失败！', e)
