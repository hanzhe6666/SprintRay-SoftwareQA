...
class Operation(Enum):
    """
       ...
       iframe                      >               跳转到iframe
       web_is_element_displayed    >               等待元素出现
    ...
    web_operation = ['input', 'click', 'text', 'submit', 'scroll', 'clear',
                     ...
                     'web_url', 'web_title', 'web_html_content', 'iframe','web_is_element_displayed']
...
class Web(Base):
    ...
    def web_judge_execution(self, types, locate, operate=None, text=None, notes=None, index=None, wait=None):
        ...
        if operate is None:
            ...
            elif operate == 'web_is_element_displayed':  # 检查元素是否存在
                self.sleep(wait)
                logger.debug(notes)
                return self.web_is_element_displayed(types=types, locate=locate)
